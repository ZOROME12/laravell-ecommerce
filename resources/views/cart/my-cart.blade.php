...
@else
    <div class="bg-white shadow rounded-lg p-4">
        @php
            $hasStockIssue = false; // Track if any item exceeds stock
        @endphp
            ...
            <div class="flex items-center justify-between border-b py-4">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">Available Stock: {{ $stock }}</p>
                        ...
                        <div class="flex items-center space-x-2 mt-1">
                            ...
                            <!-- Increase Button (with stock limit) -->
                            @if ($item->quantity < $stock)
                                <form action="{{ route('cart.update', $item) }}" method="POST" class="flex items-center">
                                    @csrf
                                    @method('PATCH')
                            ...
                            @else
                                <button disabled class="px-2 py-1 bg-gray-100 text-gray-400 rounded cursor-not-allowed">+</button>
                            @endif
                        </div>

                        <p class="text-[var(--secondary)] font-bold mt-1">
        ...
        <div class="mt-4">
            ...
            <a href="{{ route('order.place') }}" 
               class="inline-block mt-4 px-6 py-3 rounded-lg font-semibold text-white 
                      {{ $hasStockIssue ? 'bg-gray-400 cursor-not-allowed pointer-events-none' : 'bg-red-600 hover:bg-red-700' }} 
                      shadow-md hover:shadow-lg hover:scale-105 
                      transition duration-300 ease-in-out">
                Place Order
            </a>
            @if($hasStockIssue)
                <p class="text-red-600 mt-2">Some items exceed available stock. Adjust quantity to proceed.</p>
            @endif
        </div>
    </div>
@endif
